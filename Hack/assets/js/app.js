    ...
    .then((querySnapshot) => {
      querySnapshot.forEach((doc) => {
        if (doc.data().Id === uid) {
          ...
          let months = Math.floor(finalDate / (1000 * 3600 * 24 * 30.5));
     ...
     <div class="cardsDiv">
           ...
           hours === 0
             ? `${
                 seconds < 60
                   ? `${
                       seconds < 30
                         ? `<span class='pr-2'>Upload: Just Now </span>`
                         : `<span class='pr-2'>Upload: ${seconds} ${sec} ago </span>`
                     }`
                   : `<span class='pr-2'>Upload: ${minutes} ${min} ago </span>`
               }`
             : `${
                 hours > 24
                   ? days > 30
                     ? `<span class='pr-2'>Upload: ${months} ${month} ago </span>`
                     : `<span class='pr-2'>Upload: ${days} ${day} ago </span>`
                   : `<span class='pr-2'>Upload: ${hours} ${hour} ago </span>`
               }`
         }
         ...
         <span>Price : ${Number(doc.data().price).toLocaleString()} Rs</span>
         </div>
         <h4 class='cardH4'>${doc.data().ItemName}</h4> 
         <p id="ordMSG></p>
         <p style="color: green;" id="ordMsg"></p>
         <p class="card-text card-para">${
           doc.data().ItemDesc ? doc.data().ItemDesc : ""
         } </p>
         <a href="javascript:void(0)" class="orderBtn ${
           doc.id
         }" id="orderBtn" onclick="delfunc(this)">Delete <i class="fa-solid fa-trash"></i></a>
         <!-- <a href="javascript:void(0)" class="orderBtn ${
           doc.id
         }" id="orderBtn" onclick="orderfunc(this)">Add To Cart <i class="fa-solid fa-plus"></i></a> -->
         </div>
     </div>
    `;
  ...
  db.collection("products")
    ...
    .then((querySnapshot) => {
      ...
      <div class="buyCards">
          ...
          <h5 class="price">Price : ${Number(
            doc.data().price
          ).toLocaleString()} Rs</h5>
          <div class="plusMinus"><a href="javascript:void(0)">-</a><input type="number" value="1" class="productCount" disable/><a href="javascript:void(0)" onclick="addCount()">+</a></div>
          <div class="addtoCartAnchorDiv"><a href="javascript:void(0)" class="addToCartAnchor ${
            doc.id
          }" onclick="orderfunc(this)">Add to Cart</a></div>
      </div>
  </div>
    `;
...
function getCartItems() {
  ...
  // let size = priceArr.length;
  // findSum(priceArr, size);
  ...
  proRef.get().then((snapshot) => {
    snapshot.forEach((doc) => {
      ...
      docRef.get().then((docs) => {
        docs.data().myCart.forEach((ids) => {
          if (ids === doc.id) {
            ...
            priceArr.push(Number(doc.data().price));
            // console.log(findSum(priceArr,priceArr.length));
            let totalSum = findSum(priceArr, priceArr.length);
            cartSummaryTotalPrice.innerHTML = `Rs ${totalSum.toLocaleString()}`;
            let cloneRows = `
          <div class="row bb">
      ...
      <span>Subtotal</span>
      <span class="subtotalPrice">Rs <span class="sbPrice">${
        doc.data().price
      }</span></span>
      </div>
      `;
...
function delCartItem(cartItemId) {
  ...
  docRef.get().then((docs) => {
    ...
    let delCart = docs.data().myCart;
    filteredArray = delCart.filter(function(e) { return e !== dltId });
  ...
  // filteredArray.length>0 && db.collection("users")
  // .doc(uid)
  // .update(
  //   {
  //     myCart: [...filteredArray],
...
firebase.auth().onAuthStateChanged((user) => {
  if (user) {
    ...
    docRef
      ...
      .then((doc) => {
        if (doc.exists) {
          cartCount.forEach((cartCounts) => {
            cartCounts.innerHTML = doc.data().myCart.length.toString();
          });
...
function addCount() {
  ...
  productCount.forEach(function (products) {
